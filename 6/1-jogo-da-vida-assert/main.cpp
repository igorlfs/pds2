...
int main() {
    ...
    assert(numero_de_iteracoes > 0);

    int numero_de_linhas;
    cin >> numero_de_linhas;
    assert(numero_de_linhas > 0);
    int numero_de_colunas;
    cin >> numero_de_colunas;
    assert(numero_de_colunas > 0);

    JogoDaVida jogo(numero_de_linhas, numero_de_colunas);
    int linha, coluna;
    while (cin >> linha >> coluna) {
        assert(linha >= 0 && linha < numero_de_linhas);
        assert(coluna >= 0 && coluna < numero_de_colunas);
