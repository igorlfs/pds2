...
public:
  ...
  bool contains(double x, double y) const {
    // a 1ª coordenada está após o canto esquerdo, e antes do deslocamento
    // horizontal total a 2ª coordenada está acima do canto inferior, e abaixo
    // do deslocamento vertical total
    return ((x >= _x && x <= _x + _l) && (y >= _y && y <= _y + _a));
  }
...
public:
   ...
   */
  bool contains(double x, double y) const {
    // um ponto está contido em um círculo se a sua distância em relação ao
    // centro é menor (ou igual) ao raio
    return (_r >= sqrt(pow((x - _x), 2) + pow((y - _y), 2)));
  }
...
public:
  CompositeFigure(const Figure &lhs, const Figure &rhs, const char &operation)
      : a(&lhs), b(&rhs), c(operation) {}
  bool contains(double x, double y) const {
    if (c == '+') {
...
Figure *operator+(const Figure &lhs, const Figure &rhs) {
  CompositeFigure *s = new CompositeFigure(lhs, rhs, '+');
  return s;
}
 ...
 */
Figure *operator-(const Figure &lhs, const Figure &rhs) {
  CompositeFigure *d = new CompositeFigure(lhs, rhs, '-');
  return d;
}
