...
public:
   ...
   */
  void add(T e) { this->areas.push_back(e); }
  /**
  ...
  int getTotalArea() const {
    int total = 0;
    for (unsigned i = 0; i < this->areas.size(); ++i) {
      total += this->areas[i].getArea();
    }
    return total;
  }
};
...
public:
  ...
  int getArea() const { return this->lado * this->altura; }
  /**
...
public:
  ...
  int getArea() const {
    int area = 0;
    for (auto i = this->comodos.begin(); i != this->comodos.end(); ++i) {
      area += i->second;
    }
    return area;
  }
